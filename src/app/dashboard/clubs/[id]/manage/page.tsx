...
export default function ClubManagePage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "hackathons" | "participants" | "settings"
  >("overview");
  const [selectedHackathon, setSelectedHackathon] = useState<number | null>(
    null,
  );
  const [, setShowCreateHackathon] = useState(false);
  const [filterStatus, setFilterStatus] = useState<
    "all" | "pending" | "accepted" | "rejected" | "waitlisted"
  >("all");
    ...
    const fetchData = async () => {
      ...
      const mockHackathons: Hackathon[] = [
        ...
        },
        {
          ...
          description:
            "Create innovative mobile applications for modern challenges.",
          status: "upcoming",
      ...
      const mockTeams: Team[] = [
        {
          ...
          responses: {
            ...
            "Describe your team's experience with AI/ML":
              "We have 2+ years of experience in machine learning and have won several competitions.",
        ...
        },
        {
          ...
          responses: {
            ...
            "Describe your team's experience with AI/ML":
              "Beginner level but very motivated to learn.",
      ...
      const mockIndividuals: Individual[] = [
        {
          ...
          motivation:
            "I'm passionate about AI and want to contribute to innovative solutions. I have experience in Python and machine learning.",
          responses: {
            "What is your team name?": "Looking for team",
            "Describe your team's experience with AI/ML":
              "2 years of experience in data science and ML",
        ...
        },
        {
          ...
          motivation:
            "Experienced developer looking to join a strong team for this hackathon.",
          responses: {
            "What is your team name?": "Looking for team",
            "Describe your team's experience with AI/ML":
              "Advanced level with 4 years of experience",
            "What technologies will you use?":
  ...
  const handleStatusUpdate = (
    ...
    if (type === "team") {
      setTeams((prev) =>
        prev.map((team) =>
          team.id === id
            ? { ...team, status: status as Team["status"] }
            : team,
        ),
      );
    } else {
      setIndividuals((prev) =>
        prev.map((ind) =>
          ind.id === id
            ? { ...ind, status: status as Individual["status"] }
            : ind,
        ),
      );
    }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "accepted":
        return (
          <span className="px-2 py-1 bg-[#00C853]/20 text-[#00C853] text-xs font-medium rounded-full flex items-center">
      ...
      case "rejected":
        return (
          <span className="px-2 py-1 bg-[#FF3B30]/20 text-[#FF3B30] text-xs font-medium rounded-full flex items-center">
      ...
      case "waitlisted":
        return (
          <span className="px-2 py-1 bg-[#FF9500]/20 text-[#FF9500] text-xs font-medium rounded-full flex items-center">
      ...
      case "pending":
        return (
          <span className="px-2 py-1 bg-[#00CFFF]/20 text-[#00CFFF] text-xs font-medium rounded-full flex items-center">
            <Clock size={12} className="mr-1" />
            Pending
          </span>
        );
  ...
  const filteredIndividuals = individuals.filter((individual) => {
    const matchesHackathon =
      !selectedHackathon || individual.hackathonId === selectedHackathon;
    const matchesStatus =
      filterStatus === "all" || individual.status === filterStatus;
    const matchesSearch =
        ...
        <div className="flex space-x-8">
          {[
            ...
            <button
              ...
              onClick={() =>
                setActiveTab(
                  tab.id as "overview" | "hackathons" | "participants" | "settings",
                )
              }
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-2xl font-bold text-white">
                    {teams.filter((t) => t.status === "pending").length +
                      individuals.filter((i) => i.status === "pending").length}
                  </span>
                </div>
                <h3 className="text-white font-semibold mb-1">
                  Pending Reviews
                ...
                >
                  <div className="relative h-48">
                    <div
                      ...
                      style={{
                        backgroundImage: `url(${hackathon.coverImage})`,
                      }}
                    >
                  ...
                  <div className="p-6">
                    ...
                    <div className="space-y-3 mb-4">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-white">
                          {hackathon.teams}/{hackathon.maxTeams}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        {team.status === "pending" && (
                          <div className="flex items-center space-x-2">
                            <button
                              onClick={() =>
                                handleStatusUpdate("team", team.id, "accepted")
                              }
                            ...
                            <button
                              onClick={() =>
                                handleStatusUpdate("team", team.id, "rejected")
                              }
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-[#A0A0A0] text-sm">
                            {individual.email} • {individual.experience} •{" "}
                            {new Date(
                              individual.submittedAt,
                            ).toLocaleDateString()}
                          </p>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {individual.skills.map((skill, index) => (
                            <span
                              key={index}
                              className="px-2 py-1 bg-[#1E3C72]/20 text-[#00CFFF] text-xs font-medium rounded-full"
                            >
                              {skill}
                            </span>
                          ))}
